...
class Secure_Controller extends CI_Controller 
{
	...
	*/
	public function __construct($module_id = NULL, $submodule_id = NULL)
	{
		...
		// echo !$model->has_module_grant($module_id, $logged_in_employee_info->person_id) ? "First condition failed" : "First condition passed";
		// exit;
		...
		$data['inventory_total'] = count((array)$this->Item->get_inventory_total($user_info->branch_id));
		$data['stock_total'] = count((array)$this->Item->get_stock_total($user_info->branch_id));
		$data['reorder_total'] = count((array)$this->Item->get_reorder_total($user_info->branch_id));
		$expiry_check= $this->Item->get_expiry_total($user_info->branch_id);
		
		
		$data['expiry_total'] = count((array)$this->check_if_expire($expiry_check));
		$data['branch'] =$this->CI->Stock_location->get_location_name($user_info->branch_id);
		$data['controller_name'] = $module_id;
		if (count((array)$branch_sales) > 0) {
	...
	public function check_if_expire($expiry_check){
		$expiry_total = null;
		foreach((array)$expiry_check as $row=>$value){
