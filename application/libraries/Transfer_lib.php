...
class Transfer_lib
{
	...
	{
		if (!$this->CI->session->userdata('lpo_supplier')) {
			$this->set_supplier(-1);
		}

		return $this->CI->session->userdata('lpo_supplier');
	}

	public function set_supplier($supplier_id)
	{
		$this->CI->session->set_userdata('lpo_supplier', $supplier_id);
	}

	public function remove_supplier()
	{
		$this->CI->session->unset_userdata('lpo_supplier');
	}

	public function get_mode()
	{
		if (!$this->CI->session->userdata('lpo_mode')) {
			$this->set_mode('receive');
		}

		return $this->CI->session->userdata('lpo_mode');
	}

	public function set_mode($mode)
	{

		$this->CI->session->set_userdata('lpo_mode', $mode);
	...
	{
		$this->CI->session->unset_userdata('lpo_mode');
	}

	public function get_stock_source()
	{
		if (!$this->CI->session->userdata('lpo_stock_source')) {
			$this->set_stock_source($this->CI->Stock_location->get_default_location_id());
		}

		return $this->CI->session->userdata('lpo_stock_source');
	}

	public function get_comment()
	{
		// avoid returning a NULL that results in a 0 in the comment if nothing is set/available
		$comment = $this->CI->session->userdata('lpo_comment');
	...
	{
		$this->CI->session->set_userdata('lpo_comment', $comment);
	}

	public function clear_comment()
	{
		$this->CI->session->unset_userdata('lpo_comment');
	}

	public function get_reference()
	{
		return $this->CI->session->userdata('lpo_reference');
	}

	public function set_reference($reference)
	{
		$this->CI->session->set_userdata('lpo_reference', $reference);
	}

	public function clear_reference()
	{
		$this->CI->session->unset_userdata('lpo_reference');
	}

	public function is_print_after_sale()
	{
		return $this->CI->session->userdata('lpo_print_after_sale') == 'true' ||
			$this->CI->session->userdata('lpo_print_after_sale') == '1';
	}

	public function set_print_after_sale($print_after_sale)
	{
		return $this->CI->session->set_userdata('lpo_print_after_sale', $print_after_sale);
	}

	public function set_stock_source($stock_source)
	{
		$this->CI->session->set_userdata('lpo_stock_source', $stock_source);
	}

	public function clear_stock_source()
	{
		$this->CI->session->unset_userdata('lpo_stock_source');
	}

	public function get_stock_destination()
	{
		if (!$this->CI->session->userdata('lpo_stock_destination')) {
			$this->set_stock_destination($this->CI->Stock_location->get_default_location_id());
		}

		return $this->CI->session->userdata('lpo_stock_destination');
	}

	public function set_stock_destination($stock_destination)
	{
		$this->CI->session->set_userdata('lpo_stock_destination', $stock_destination);
	}

	public function clear_stock_destination()
	{
		$this->CI->session->unset_userdata('lpo_stock_destination');
	}

	public function add_item($item_id, $item_location = NULL, $receiving_quantity = NULL)
	{
	...
	public function edit_item($line, $receiving_quantity)
	{
	...
	public function get_total()
	{
		...
		foreach ($this->get_cart() as $item) {
			...
			$total = bcadd($total, $this->get_item_total($item['receiving_quantity'], $item['price'], $item['discount']));
		}
