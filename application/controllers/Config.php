...
class Config extends Secure_Controller
{
	...
	*/
	...
	public function index($err = null)
	{
		...
		$data['employee_location'] = $this->Employee->get_logged_in_employee_info()->branch_id;
		$locations = array();
		foreach ($data['stock_locations'] as $loc) {
			$locations[$loc['location_id']] = $loc['location_name'];
		}
	...
	public function register_loc(){
	    echo json_encode($this->Stock_location->register_online());
    }
    public function get_locs(){
        echo json_encode($this->Stock_location->get_updated_locations());
    }
	public function edit_location(){
	    if(!$this->input->is_ajax_request()){
	        echo json_encode(['message'=>'Wrong location, sorry']);
	        exit;
        }
	    ...
	    switch($type){
            case 'name':
                ...
                echo json_encode($this->Stock_location->update_name($name));
                break;
            case 'phone':
                $name = $this->input->post('stock_number');
                echo json_encode($this->Stock_location->update_phone($name));
                break;
            case 'address':
                $name = $this->input->post('stock_address');
                echo json_encode($this->Stock_location->update_address($name));
                break;
            case 'mail':
                $name = $this->input->post('stock_email');
                echo json_encode($this->Stock_location->update_mail($name));
                break;
            default:
                echo json_encode(['message'=>'Wrong input, sorry']);
                break;
        }
        exit();
    }
	public function  update_app_config_extra(){
        $employee = $this->Employee->get_logged_in_employee_info();
	    $data = [
	      'minimum_sale_value'=>parse_decimals($this->input->post('min_sale')),
            'customer_details_mandated'=>$this->input->post('customer_mandated'),
            'last_edited_by'=>$employee->person_id,
        ];
	    $update_clause = [
            'company_id'=>$employee->branch_id,
            'company_branch_id'=>$employee->branch_id
        ];
	    $response = ['error'=>"Error updating your settings"];
	    if($this->Appconfig->update_extra_config($data,$update_clause)){
	        $response = ['message'=>"Settings updated"];
        }
        echo json_encode($response);
        exit();
    }
    public function set_extra_app_config(){
	    $employee = $this->Employee->get_logged_in_employee_info();
        $data = [
            'minimum_sale_value'=>parse_decimals($this->input->post('min_sale')),
            'customer_details_mandated'=>$this->input->post('map_sales_to_customers'),
            'last_edited_by'=>$employee->person_id,
            'company_id'=>$employee->branch_id,
            'company_branch_id'=>$employee->branch_id
        ];
        $response = ['error'=>"Error updating your settings"];
        if($this->Appconfig->add_extra_config($data)){
            $response = ['message'=>"Settings updated"];
        }
        echo json_encode($response);
        exit();
	...
	public function sales_settings()
	{
		$sales = array("unit_price_markup" => $this->input->post('unit'), "wholesale_price_markup" => $this->input->post('wholesale'));


		$status = $this->Appconfig->batch_save($sales);
	...
	*/
	...
	private function backup_error($error_det){
	    $this->load->view('configs/manage',['c_error'=>true,'error_det'=>['error_message'=>$error_det]]);
    }

	public function backup_db($non_sup = 0)
	{
        $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        if ($this->Employee->has_module_grant('config', $employee_id) || $non_sup == 1) {
//            $this->load->dbutil();
...
//            $backup = $this->dbutil->backup($prefs);
            ...
            $filename = $db_name . "-" . date("Y-m-d_H-i-s") . ".sql.gz";
//            $filename = $db_name . "-" . date("Y-m-d_H-i-s") . ".sql";
//            $cmm = "(mysqldump --user=$db_user --password=$db_pass $db_name > db_backup.sql) 2&1";
            $mime = "application/x-gzip";
//
            header( "Content-Type: " . $mime );
            header( 'Content-Disposition: attachment; filename="' . $filename . '"' );
////            $cmd = "mysqldump -u $db_user --password=$db_pass --no-create-info --complete-insert $db_name | gzip --best";
            $cmd = "mysqldump --opt -h localhost -u $db_user  $db_name | gzip -c> $filename";
            if($pass){
                $cmd = "mysqldump --opt -h localhost -u $db_user --password=$db_pass  $db_name | gzip -c> $filename";
            }
//            $cmd = "mysqldump --opt -h localhost -u $db_user --password=$db_pass  $db_name > $filename";
            passthru( $cmd );
            ob_flush();
            if($non_sup !== 1){
                $this->index();
...
//            exit(0);
//            try{
//                $dump = new IMysqldump\Mysqldump('mysql:host=localhost;dbname='.$db_name,$db_user,$db_pass,
//                    ['compress'=>IMysqldump\Mysqldump::GZIP]);
//                $dump->start($filename);
//                $this->index();
//            }catch (\Exception $e){
//                $err =  'Backup error: '.$e->getMessage();
//                $this->index($err);
//            }
...
//            force_download($file_name, $backup);
	...
	/**
	 * This is function set all the product quantity to a particular quantity.
	 * 
	 */
	public function zero_all_quantity()
	{
		...
		//fetch all the product quantity
		$inventory = array();
		$quantities_update = array();
		$quantities = $this->Item_quantity->fetch_all_non_zero_quantity($location);
		foreach ($quantities as $quantity) {
			if ($quantity->quantity != 0) {
				//system was reading negative quantity. 
				$quantities_update[] = array(
					"id" => $quantity->id,
					"quantity" => 0
				);
				$inventory[] = array(
					...
					"trans_comment" => "Zeroing all items with non-zero quantities",
					"trans_location" => $quantity->location_id,
					"trans_inventory" => $quantity->quantity,
					"trans_remaining" => 0
				);
			}
		}
		//update
		$this->Item_quantity->zero_all_quantities_in_batch($quantities_update);
		$this->Inventory->insert_batch($inventory);


		echo json_encode(array("status" => true, "message" => "You have successfully zeroed " . count($inventory) . " items"));
	}

	public function save_expiry_period()
	{
		$reorder_save_data = array(
			'reorder_default' => $this->input->post('reorder_default'),
		);

		$result = $this->Appconfig->reorder_level_save($reorder_save_data);
		$success = $result ? TRUE : FALSE;
