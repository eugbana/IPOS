        ...
        <div id="register_wrapper">
            ...
            <div class="panel-body form-group">
                <ul>
                    ...
                    <li class="pull-left">
                        <?php echo form_dropdown('mode', $modes, $mode, array('onchange' => "setSalesMode($(this));",'id'=>"sales_mode_drop", 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-default btn-sm', 'data-width' => 'fit')); ?>
                    </li>
                    <?php
                    ...
                    if (count($stock_locations) > 1) {
                        ...
                        <li class="pull-left">
                            <?php echo form_dropdown('stock_location', $stock_locations, $stock_location, array('onchange' => "setStockLoc($(this));", 'class' => 'selectpicker show-menu-arrow', 'data-style' => 'btn-default btn-sm', 'data-width' => 'fit')); ?>
                        </li>
                        <?php
                    }
                    ?>
            ...
            <table class="sales_table_100" id="register">
                ...
                <tr>
                    ...
                    <th style="width: 32%;"><?php echo $this->lang->line('sales_item_name'); ?></th>
                        ...
                        <?php if ($item['reference'] == 0) { ?>
                            ...
                            <tr>
                                ...
                                <td style="align: center;">
                                    ...
                                    <?php echo form_hidden('itemid', $item['item_id']); ?>
                                ...
                                <td><?php echo form_dropdown('qty_type', $qtytypes, $item['qty_selected'], array('id' => 'qty_type')); ?></td>
            ...
            <div class="panel-body">
                ...
                <table class="sales_table_100" id="sale_totals">
                    <tr>
                        ...
                        <th style="width: 20%; text-align: right;"><?php echo to_currency($initial_cost); ?><p></p>
                        </th>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <th style="width: 20%; text-align: right;"><?php echo to_currency($total_vat); ?><p></p>
                        </th>
                    </tr>
                    <tr>
                        <th style="width: 20%;"><?php echo $this->lang->line('sales_sub_total'); ?></th>
                        <th style="width: 20%; text-align: right;"><?php echo to_currency($this->config->item('tax_included') ? $tax_exclusive_subtotal : $subtotal); ?><p></p>
                        </th>
                    </tr>
                        ...
                        <?php } else { ?>
                            ...
                            <table class="sales_table_100">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        //echo form_input(array('name' => 'amount_tendered', 'id' => 'amount_tendered', 'class' => 'form-control input-sm giftcard-input', 'disabled' => true, 'value' => to_currency_no_money($amount_due), 'size' => '5', 'tabindex' => ++$tabindex));
                                        ?>
                                    </td>
                                </tr>
                            ...
                            <?php else : ?>
                                <?php if ($discount_approved == FALSE) : ?>
                                    <div class='btn btn-sm btn-success pull-right' id='verify_discount' tabindex='<?php echo ++$tabindex; ?>'><span class="glyphicon glyphicon-credit-card">&nbsp</span>Approve</div>
                                <?php else : ?>
                                    <div class='btn btn-sm btn-success pull-right' id='add_payment_button' tabindex='<?php echo ++$tabindex; ?>'><span class="glyphicon glyphicon-credit-card">&nbsp</span><?php echo $this->lang->line('sales_add_payment'); ?></div>
                                <?php endif; ?>
                            <?php endif; ?>
...
<script type="text/javascript">
    ...
    function setSalesMode(el){
        let sales_data = JSON.parse(localStorage.getItem('sales_data'));
        if (sales_data === undefined || sales_data === null || sales_data === 0)
        {
            // items is undefined, null, [] or '' (empty string)
            sales_data = {};
        }
        sales_data['sale_mode'] = el.val();
        localStorage.setItem('sales_data',JSON.stringify(sales_data));
    }
    function setStockLoc(el){
        let sales_data = JSON.parse(localStorage.getItem('sales_data'));
        if (sales_data === undefined || sales_data === null || sales_data === 0)
        {
            // items is undefined, null, [] or '' (empty string)
        ...
        sales_data['stock_location'] = el.val();
        localStorage.setItem('sales_data',JSON.stringify(sales_data));
    }
    $(document).ready(function() {
        let sales_data = JSON.parse(localStorage.getItem('sales_data'));
        const item_module_allowed = <?php  if($items_module_allowed) {
                                        echo 1;}else{echo 0;}?>
        if (sales_data === undefined || sales_data === null || sales_data === 0)
        {
            // items is undefined, null, [] or '' (empty string)
            sales_data = {};
            sales_data['sale_type'] = 'retail';
            sales_data['sale_mode'] = $('#sales_mode_drop').val();
            localStorage.setItem('sales_data',JSON.stringify(sales_data));
        }
        ...
        $("#item").autocomplete({
            ...
            select: function(a, ui) {
                ...
                $.post('<?php echo site_url($controller_name . "/add/1"); ?>',{'sale_mode':sales_data['sale_mode'],'item':ui.item.value},function (data) {
                    sales_data = JSON.parse(localStorage.getItem('sales_data'));
                    const cart_item = data.cart_items;
                    console.log('items:',cart_item);
                },'json');
                return false;
            }
        });
        ...
        $('[name="price"],[name="quantity"],[name="discount"],[name="description"],[name="serialnumber"],[name="reminder_value"],[name="no_of_days"]').focusout(function() {
            $(this).parents("tr").prevAll("form:first").submit()
        });
        $('[name="qty_type"],[name="time_started"]').change(function() {
            $(this).parents("tr").prevAll("form:first").submit()
        });
