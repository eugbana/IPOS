					...
					?>
							...
							if($item['in_stock'] <=0 ){?>
								...
								<tr>
									...
									<td><?php if($item['stock_name'] == $this->lang->line('wholesale')){ echo to_currency($item['wholeprice']*$item['quantity']-$item['wholeprice']*$item['quantity']*$item['discount']/100);}else{echo to_currency($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100);} ?></td>
									<td><?php echo form_dropdown('qty_type', $qtytypes, $item['qty_selected'], array('id'=>'qty_type')); ?></td>
									<td><?php $piller=$item['item_id'];$liner=$item['line']; echo anchor($controller_name."/add_sale_pill/$piller", '<span class="glyphicon glyphicon-time"></span>');?></td>
									<?php ?>
								</tr>
