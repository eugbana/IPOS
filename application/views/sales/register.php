				...
				<tbody id="cart_contents">
					<?php if (count($cart) == 0) { ?>
						...
						<?php } else {
							...
							<?php
										session_start();
										$warnin = $this->lang->line('He_don_finish');
										$_SESSION['wari'] = $warnin;
										$webe =  site_url($controller_name . "/delete_ite/$line");
									...
									<tr>
										...
										<?php
											...
											<td><?php if ($item['stock_name'] == $this->lang->line('wholesale')) {
																		echo form_input(array('name' => 'price', 'class' => 'form-control input-sm', 'value' => to_currency_no_money($item['wholeprice']), 'tabindex' => ++$tabindex));
																	} else {
																		echo form_input(array('name' => 'price', 'class' => 'form-control input-sm', 'value' => to_currency_no_money($item['price']), 'tabindex' => ++$tabindex));
																	} ?></td>
										<?php
														} else {
															?>
