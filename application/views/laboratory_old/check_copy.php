		...
		<div class="container">
			<div class="row">
				<div class="col-xs-12">
					...
					<?php echo form_open("laboratory/lab_result_receipt", array('class' => 'form-horizontal ajax-form', 'id' => 'sub')); ?>
					<div class="col-xs-1"><button class='btn btn-info btn-sm' id="submitButton">
								Submit
					</button></a></div>
					<?php echo form_close(); ?>

					<!-- <div class="col-xs-1"><a href='<?php echo site_url("laboratory/lab_result_receipt"); ?>' id="sub"><button class='btn btn-info btn-sm' id="submitButton">
								Submit
					</button></a></div> -->
				</div>
				...
				foreach ($cart as $row => $value) { ?>
					...
					<?php } elseif (strstr($value['test_subgroup'], 'Microbiology') && $value['test_name'] != 'Urinalysis') { ?>
						<?php if ($value['reference'] == 0) { 
							...
							?>
							<?php //echo form_open($controller_name . "/edit_item/$row", array('class' => 'form-horizontal ajax-form', 'data-line' => $row, 'data-id' => 'cart_' . $row)); ?>
							<?php echo form_open($controller_name . "/edit_item/$l", array('class' => 'form-horizontal ajax-form', 'data-line' => $l, 'data-id' => 'cart_' . $l)); ?>
							<tr>
								<td colspan="7" style="border-top:1px solid black;">&nbsp;&nbsp;</td>
							</tr>
							<tr>
					...
					<?php } else { ?>
						...
						<?php if (strstr($value['test_name'], 'Urinalysis')) { ?>
							...
							?>
							<?php //echo form_open($controller_name . "/edit_item/$row", array('class' => 'form-horizontal', 'data-id' => 'cart_' . $row)); ?>
							<?php echo form_open($controller_name . "/edit_item/$row", array('class' => 'form-horizontal', 'data-line' => $row, 'data-id' => 'cart_' . $row)); ?>
							<tr>
								<td colspan="7" style="border-top:1px solid jude black;">&nbsp;&nbsp;</td>
							</tr>
							<tr>
								<td></td>
								<td>
									<h4><b><?php echo $value['test_name'] . ' row: '. $let ?></b></h4>
								</td>
								<td><?php echo form_input(
										array(
											'name' => 'test_comment' . $row,
								...
								<td colspan="4">
									...
									echo '<button class="btn btn-primary btn-sm update-btn" tabindex="' . $tabIndex++ . '" data-lets="' . implode(" ", $lines) . '" data-line="' . $unline['line'] . '"  data-let="' . $row . '">
										Enter
									</button>';

									// echo '<button class="btn btn-primary btn-sm update-btn" tabindex="' . $tabIndex++ . '" data-lets="' . implode(" ", $lines) . '" data-line="' . $l . '"  data-let="' . $l . '">
									// 	Enter
									// </button>';
...
<script>
	$(document).ready(function() {
		...
		$(".update-btn").click(function() {
			...
			console.log('lets data', update_btn.data("lets"));
			...
			console.log('test lines', test_lines);
		...
		//the values on the datalist are value previously set  while using the system to take record test.
		...
		//this stub takes care of storing values typed into some fields so they can be remember for next record inputs
		$("input").on('change', function() {
			var ele = $(this);
			if (ele.data('datalist')) { //check the html(top) part of this document, I have added the data-datalist attribute to some input element that are used to capture tests records. notice that the value of the data-datalist attribute are unique to the for both the test and that particular line in a  test
				var valu = ele.val().trim(); //get the value just typed into this input
				var eleName = ele.data('datalist'); //this name if unique for a test and a line in a test
				if (valu) { //making sure something is really typed into the text box

					var storeItem = localStorage.getItem(eleName, null); //check whether something is already stored with the name in the localStorage
					if (!storeItem) {
						//not existing another record with this element name in the localstorage 
						//make a new localstorage entry for this name
						//notice that entries are array. this will allow to store many values for each input name
						var insertItems = [];
						insertItems[0] = valu;
						localStorage.setItem(eleName, JSON.stringify(insertItems));
					} else {
						var storeItems = JSON.parse(storeItem);
						//already existing other record(s) for this element
						//check if this particular value is alredy in the array for the element in the localstorage
						var existing = storeItems.indexOf(valu) > -1;
						//var existing = storeItems.include(value);//another way of checking existing of element in array for es6


						if (!existing) {
							//if not existing

							var newLen = storeItems.unshift(valu);

							if (newLen > 20) { //the array should not contain more than 20 elements

								storeItems.pop(); //remove the last element
							}
		...
		$('#submitButton').click(function(e){
			...
			$('#sub').submit();
			// $('#submitButton').find('a').trigger('click');
		});
