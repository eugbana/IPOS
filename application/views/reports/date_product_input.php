	...
	<div class="content">
		...
		?>
			...
			<div class="form-group form-group-sm">
				...
				<div class="col-xs-3">
					<?php echo form_input(array('name' => 'customer_id', 'id' => 'customer_id', 'placeholder'=>'all', 'class' => 'form-control input-sm', 'size' => '50', 'tabindex' => ++$tabindex)); ?>
				</div>
			</div>
			...
			<!-- <input type="hidden" name="stock_location" value="<?=$current_location?>"/> -->
				...
				<div id='report_stock_location' class="col-xs-3">
					<?php
					//  echo form_dropdown('stock_locaion', $stock_locations, $current_location, array('id' => 'locaton_i', 'class' => 'form-control','disabled'=>'disabled')); ?>
                    <input type="hidden" name="stock_location" id="location_id" value="<?=$current_location?>"/>
				</div>
			</div>
		<?php
		...
		?>
			...
			<div class="form-group form-group-sm">
				...
				<div id='report_receiving_from_type' class="col-xs-3">
					<?php echo form_dropdown('from_branch', $stock_locations, $default_from_location, array('id' => 'from_branch', 'class' => 'form-control')); ?>
				</div>
			</div>
			<div class="form-group form-group-sm">
				...
				<input type="hidden" name="stock_location" id="location_id" value="<?=$current_location?>"/>
					<?php //echo form_dropdown('to_branch', array_merge(array('all' => 'All'), $stock_locations), 'all', array('id' => 'to_branch', 'class' => 'form-control')); ?>
				</div>
			</div>
		<?php
		}
	...
	$(document).ready(function() {
		...
		$("#generate_report").click(function() {
			...
			?>
				...
				window.location = ["<?php echo site_url('reports/print_filtered_report_product_specific_items_receivings');  ?>", start_date, end_date, $("#input_type").val() || 0, $("#location_id").val(), $('#employee_id').val(), $("#supplier").val(), $("#item").val() === '' ? 'all' : $("#item").val()].join("/");
			<?php
			} elseif ($mode == 'sale') {
				//JUDE HERE
			?>
				window.location = ["<?php echo site_url('reports/print_filtered_report_items_product_specific');  ?>", start_date, end_date, $("#employee_id").val(), $("#location_id").val(), $("#sale_type").val() || 0, $("#credit").val(), $("#vat").val(), $("#customer_id").val() === '' ? 'all' : $("#customer_id").val(), $("#discount").val(), $("#payment_type").val(), $("#item").val() === '' ? 'all' : $("#item").val()].join("/");
			<?php
			} elseif ($mode == 'transfer') {
			?>
