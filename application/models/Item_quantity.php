...
class Item_quantity extends CI_Model
{
    ...
    public function save_batch($location_detail, $item_id, $location_id, $batch_no)
    {
        if (!$this->exists_batch($item_id, $location_id, $batch_no)) {
    ...
    public function get_item_batch($item_id, $location_id, $batch_no)
    {
        ...
        if (empty($result) == TRUE) {
            ...
            //Get all the fields from items table (TODO to be reviewed)
            foreach ($this->db->list_fields('item_expiry') as $field) {
                $result->$field = '';
            }
