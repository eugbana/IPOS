...
class Labor extends CI_Model
{
	...
	*/
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'items.name', $order = 'asc')
	{
		...
		$this->db->select('MAX(items.expiry) as expiry');
	...
	*/
	...
	public function lab_save(&$item_data, $item_id = FALSE)
	{
		if(!$item_id || !$this->exists($item_id, TRUE))
		{
			if($this->db->insert('laboratory_test', $item_data))
			{
				$item_data['test_id'] = $this->db->insert_id();
		...
		return $this->db->update('laboratory_test', $item_data);
	}
	...
	*/
	...
	public function laboratory_items()
	{
		...
		$this->db->select('test_id,test_code,test_name, test_amount,test_type');
		$this->db->from('laboratory_test');
		
		foreach($this->db->get()->result() as $row)
		{
			$lab_items[] = array('test_id' => $row->test_id, 'test_code' => $row->test_code, 'test_name' => $row->test_name, 'test_amount' => $row->test_amount, 'test_type' => $row->test_type,'edit' => '<span class="glyphicon glyphicon-edit update" id="'.$row->test_id.'" name="edit" title = "update" data-toggle="modal" data-target="#userModal"></span>');
		}
		$results = array(
			"sEcho" => 1,
	...
	public function global_transfer_items()
	{
		$items_transfer = array();

		$this->db->select('transfer_id,transfer_type, status');
		$this->db->from('item_transfer');
		$this->db->where("status = '0'"); // standard, exclude kit items since kits will be picked up later
		
		foreach($this->db->get()->result() as $row)
		{
			$items_transfer[] = array('transfer_id' => $row->transfer_id, 'status' => $row->status,'transfer_type' => $row->transfer_type);
		}
